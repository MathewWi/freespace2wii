...
struct Freespace_context : public Freespace_base
{
	static Freespace_context * _mythis;
	Freespace_context();
...
int	Game_font ;// -1;
#ifndef NDEBUG
 int Show_player_pos ;// 0;		// debug console command to show player world pos on HUD
#endif
...
// loading background filenames
 std::string Game_loading_bground_fname[GR_NUM_RESOLUTIONS] ;// {


 std::string Game_loading_ani_fname[GR_NUM_RESOLUTIONS] ;// {

 std::string Game_title_screen_fname[GR_NUM_RESOLUTIONS] ;
 std::string Game_logo_screen_fname[GR_NUM_RESOLUTIONS] ;

// for title screens
 int Game_title_bitmap ;// -1;
 int Game_title_logo ;// -1;
...
int Game_loading_frame;//-1;
 ...
 int Game_loading_ani_coords[GR_NUM_RESOLUTIONS][2] ;// {

#ifndef NDEBUG
 int busy_shader_created ;// 0;
shader busy_shader;
#endif
...
static void static_game_loading_callback(int a)
{
	if(_mythis)
		_mythis->game_loading_callback(a);
}
...
void game_loading_callback_init()
{
	...
	{
		Game_loading_background = bm_load(Game_loading_bground_fname[gr_screen.res].c_str());
	}
	//common_set_interface_palette("InterfacePalette");  // set the interface palette


	Game_loading_ani = anim_load( Game_loading_ani_fname[gr_screen.res].c_str());
	Assert( Game_loading_ani != NULL );
	...
	Mouse_hidden = 1;
	game_busy_callback( static_game_loading_callback, (COUNT_ESTIMATE/Game_loading_ani->total_frames)+1 );	
...
static void static_game_do_dc_networking()
{
	if(_mythis)
		_mythis->game_do_dc_networking();
}
...
//				case GS_STATE_DEATH_BLEW_UP:		
		...
		case KEY_SHIFTED | KEY_ENTER: {
			...
			if(Game_mode & GM_MULTIPLAYER){
				debug_console(static_game_do_dc_networking);
			} else {				
				debug_console();
			}
...
void game_do_state_common(int state,int no_networking = 1)
{
...
#define OEM_UPSELL_SCREEN_DELAY				10000
 ...
 int Oem_upsell_bitmaps_loaded ;// 0;
 int Oem_upsell_bitmaps[GR_NUM_RESOLUTIONS][NUM_OEM_UPSELL_SCREENS];
 int Oem_upsell_screen_number ;// 0;
 int Oem_upsell_show_next_bitmap_time;

//XSTR:OFF
 char *Oem_upsell_bitmap_filenames[GR_NUM_RESOLUTIONS][NUM_OEM_UPSELL_SCREENS] ;// 
//XSTR:ON

 int Oem_normal_cursor ;// -1;
 int Oem_web_cursor ;// -1;
...
// clickable hotspot on 3rd OEM upsell screen
 int Oem_upsell3_button_coords[GR_NUM_RESOLUTIONS][4] ;// {
...
#define DEMO_UPSELL_SCREEN_DELAY				3000
 ...
 int Demo_upsell_bitmaps_loaded = 0;
 int Demo_upsell_bitmaps[GR_NUM_RESOLUTIONS][NUM_DEMO_UPSELL_SCREENS];
 int Demo_upsell_screen_number = 0;
 int Demo_upsell_show_next_bitmap_time;

//XSTR:OFF
 char *Demo_upsell_bitmap_filenames[GR_NUM_RESOLUTIONS][NUM_DEMO_UPSELL_SCREENS] = 
{
	{	"UpSell02",
		"UpSell01",
	},
...
// ----------------------------------------------------------------
 ...
 int Subspace_ambient_left_channel ;// -1;
 int Subspace_ambient_right_channel ;// -1;
...
// if the game is running using hacked data
 bool Hacked_data_check_ready ;// false;
 bool Hacked_data ;// false;
...
//		return;
	...
	*/
	...
	{
		Game_title_logo = bm_load(Game_logo_screen_fname[gr_screen.res].c_str());
		Game_title_bitmap = bm_load(Game_title_screen_fname[gr_screen.res].c_str());
...
Freespace_context * Freespace_context::_mythis = NULL;
Freespace_base * Freespace_base::_this = NULL;
	


int	Fred_running = 0;// 0;

#include "freespace_vars.hpp"
